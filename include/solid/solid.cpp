...
std::tuple<int, int, float> SOLiDModule::detectLoopClosureID( int k )
{
    int loop_id { -1 };
    ...
    auto curr_solid = solids_[k];
    auto curr_solid_vec = solids_mat_[k];
    if(k < NUM_EXCLUDE_RECENT + 1)
    {
        return std::make_tuple(-1, -1, 0.0f);
    }
    ...
    for(size_t i = 0; i < k - NUM_EXCLUDE_RECENT; ++i)
    {
    ...
    return std::make_tuple(k, loop_id, yaw_diff_rad);
}
